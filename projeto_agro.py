...
# --- INICIALIZAÇÃO DE ESTADO (BLINDAGEM CONTRA TRAVAMENTO) ---
...
# --- CAMADA 1: SIDEBAR (CONTROLE TOTAL +/-) ---
def configurar_sidebar():
    st.sidebar.image("LogoTriadeagro.png.png", use_container_width=True)
    st.sidebar.markdown("### ⚙️ Parâmetros do Projeto")
    ...
    with st.sidebar.expander("🪨 Calagem"):
        ...
        c_res = st.number_input("Reserva (kg/ha)", value=0.0, step=10.0); c_preco = st.number_input("R$/Ton Calc", value=190.0)
    ...
    with st.sidebar.expander("🧪 Fósforo"):
        ...
        f_arg = [st.number_input(f, value=v, step=0.1) for f, v in zip(["Muito Argiloso", "Argiloso", "Médio", "Arenoso"], [10.0, 8.0, 4.0, 2.0])]
        p_teor = st.number_input("Teor Adubo P %", value=21.0); p_exp = st.number_input("Exp (kg/sc) P", value=0.8); p_preco = st.number_input("R$/Ton P", value=2000.0)

    with st.sidebar.expander("🍌 Potássio"):
        k_target = st.number_input("Alvo K % CTC", value=3.2, step=0.1)
        k_exp = st.number_input("Exp (kg/sc) K", value=1.2); k_teor = st.number_input("Teor Adubo K %", value=60.0); k_preco = st.number_input("R$/Ton K", value=2800.0)

    with st.sidebar.expander("📦 Gesso"):
        g_fator = st.number_input("Fator Gesso", value=15.0, step=0.1)
        g_min = st.number_input("Mín kg/ha", value=400.0); g_max = st.number_input("Máx kg/ha", value=900.0); g_preco = st.number_input("R$/Ton Gesso", value=400.0)
