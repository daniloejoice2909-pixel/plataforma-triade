...
# --- PERSISTÊNCIA (CONTRA TRAVAMENTOS) ---
...
st.markdown("""
    ...
    .stButton>button { width: 100%; border-radius: 5px; font-weight: bold; height: 3em; background-color: #1e3d59; color: white; }
    </style>
...
def configurar_interface():
    ...
    with st.sidebar.expander("🪨 Calagem"):
        c_prnt = st.number_input("PRNT %", value=80.0, step=1.0, min_value=0.0, max_value=1000.0)
        c_cao = st.number_input("CaO %", value=36.0, step=0.1, min_value=0.0, max_value=1000.0)
        c_mgo = st.number_input("MgO %", value=9.0, step=0.1, min_value=0.0, max_value=1000.0)
        c_t_ca = st.number_input("Alvo Ca %", value=60.0, step=1.0, min_value=0.0, max_value=1000.0)
        c_t_mg = st.number_input("Alvo Mg %", value=18.0, step=1.0, min_value=0.0, max_value=1000.0)
        c_res = st.number_input("Reserva (kg/ha)", value=0.0, step=10.0, min_value=0.0, max_value=10000.0)
        c_pre = st.number_input("R$/Ton Calcário", value=190.0, step=1.0, min_value=0.0, max_value=10000.0)

    with st.sidebar.expander("🧪 Fósforo"):
        nc = [st.number_input(f"NC {f}", value=v, step=0.1, min_value=0.0, max_value=1000.0) for f, v in zip(["0-4","4-10","10-19","19-30","30-45","45-60"], [8.0, 10.0, 12.0, 15.0, 18.0, 22.0])]
        f_arg = [st.number_input(f, value=v, step=0.1, min_value=0.0, max_value=1000.0) for f, v in zip(["M. Argiloso", "Argiloso", "Médio", "Arenoso"], [10.0, 8.0, 4.0, 2.0])]
        p_t = st.number_input("Teor Adubo P %", value=21.0, step=0.1, min_value=0.0)
        p_e = st.number_input("Exp (kg/sc) P", value=0.8, step=0.01, min_value=0.0)
        p_p = st.number_input("R$/Ton Adubo P", value=2000.0, step=10.0, min_value=0.0)

    with st.sidebar.expander("🍌 Potássio"):
        k_t = st.number_input("Alvo K % CTC", value=3.2, step=0.1, min_value=0.0)
        k_e = st.number_input("Exp (kg/sc) K", value=1.2, step=0.01, min_value=0.0)
        k_te = st.number_input("Teor Adubo K %", value=60.0, step=0.1, min_value=0.0)
        k_p = st.number_input("R$/Ton Adubo K", value=2800.0, step=10.0, min_value=0.0)

    with st.sidebar.expander("📦 Gesso"):
        g_f = st.number_input("Fator Gesso", value=15.0, step=0.1, min_value=0.0)
        g_mi = st.number_input("Mín kg/ha", value=400.0, step=10.0, min_value=0.0)
        g_ma = st.number_input("Máx kg/ha", value=900.0, step=10.0, min_value=0.0)
        g_pr = st.number_input("R$/Ton Gesso", value=400.0, step=1.0, min_value=0.0)
    ...
    return {
        ...
        "fosf": {"nc": nc, "f_arg": f_arg, "teor": p_t, "exp": p_e, "pre": p_p},
...
def plot_v43_master(df, col, title, poly=None):
    ...
    fig = go.Figure()
    fig.add_trace(go.Contour(z=zi, x=xi[0], y=yi[:,0], colorscale='RdBu_r', contours=dict(showlines=False), line_width=0, colorbar=dict(thickness=15, title="V")))
...
if up_csv:
    if st.session_state['df_proc'] is None:
        st.session_state['df_proc'] = motor_v43(pd.read_csv(up_csv), sb)
    ...
    with tabs[1]: # VRT
        ...
        if st.session_state['vrt_ok']:
            for col, label, preco in [('rec_calcario', 'Calcário', sb['calc']['pre']), ('rec_potassio', 'Potássio', sb['pot']['pre']), ('rec_gesso', 'Gesso', sb['gesso']['pre'])]:
                ...
                c_i.markdown("<div class='arg-tecnico'><b>Argumento:</b> Baseado em equilíbrio estequiométrico.</div>", unsafe_allow_html=True)

    with tabs[2]: # SAÍDA FINAL
        st.subheader("Gerar Documentos e Arquivos")
        c_p, c_z = st.columns(2)
        
        # MOTOR PDF CORRIGIDO (BLINDAGEM TYPEERROR)
        pdf = FPDF(); pdf.add_page(); pdf.set_font("Helvetica", 'B', 14)
        pdf.cell(0, 10, f"RELATORIO TRIADE - {sb['p_nome'].upper()}", ln=True, align='C')
        pdf.set_font("Helvetica", '', 12); pdf.ln(5)
        pdf.cell(0, 10, f"Fazenda: {sb['f_nome']} | Talhao: {sb['t_nome']}", ln=True)
        # Convertendo explicitamente para bytes para evitar StreamlitAPIException
        ...
        with zipfile.ZipFile(buf, 'w') as z:
            for m in ["JOHN_DEERE", "TRIMBLE", "HORSCH", "CASE_IH", "STARA"]:
