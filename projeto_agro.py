...
# --- INICIALIZAÇÃO DE ESTADO (PERSISTÊNCIA E PERFORMANCE) ---
if 'db' not in st.session_state: st.session_state['db'] = {}
if 'show_fert' not in st.session_state: st.session_state['show_fert'] = False
if 'show_vrt' not in st.session_state: st.session_state['show_vrt'] = False
...
def configurar_interface():
    ...
    with st.sidebar.expander("🪨 Calagem"):
        c_prnt = st.number_input("PRNT %", value=80.0, step=1.0, min_value=0.0, max_value=120.0)
        ...
        c_preco = st.number_input("R$/Ton Calc", value=190.0, step=1.0, min_value=0.0)
    ...
    return {
        "prod": prod, "calc": {"prnt": c_prnt, "cao": c_cao, "mgo": c_mgo, "t_ca": c_t_ca, "t_mg": c_t_mg, "res": c_res, "preco": c_preco},
        "fosf": {"nc": nc, "f_arg": f_arg, "teor": p_teor, "exp": p_exp, "preco": p_preco},
        "pot": {"target": k_target, "exp": k_exp, "teor": k_teor, "preco": k_preco},
        "gesso": {"fator": g_fator, "min": g_min, "max": g_max, "preco": g_preco}
    }

# --- CAMADA 2: MOTOR AGRONÔMICO V43 ---
def motor_v43(df_raw, p):
    ...
    # 3. Potássio (Elevação 3.2% + Exportação Mandatória)
    ...
    # 4. Fósforo (NC P-rem + Crédito Solo + Exportação)
    def calc_p(row):
...
if up_csv:
    ...
    with tabs[0]: # ABA FERTILIDADE
        if st.button("🚀 GERAR MAPAS DE FERTILIDADE"): st.session_state['show_fert'] = True
        
        if st.session_state['show_fert']:
            attrs = [('ph', 'Acidez (pH)'), ('argila', 'Argila (%)'), ('v_p', 'V%'), ('prem', 'P-rem')]
