...
if st.session_state.pagina == "Entrada":
    ...
    st.header("📂 Cadastro de Fazenda e Importação")
    c1, c2 = st.columns(2)
    with c1:
        st.session_state.produtor = st.text_input("Nome do Produtor", "Danilo")
        st.session_state.fazenda = st.text_input("Nome da Fazenda", "Berneck")
    ...
    with c2:
        ...
        f_dados = st.file_uploader("Planilha de Solo (Sequência A a Y)", type=['xlsx'])

    if f_contorno and f_dados:
        st.session_state.contorno = json.load(f_contorno)
        df = pd.read_excel(f_dados)
        # MAPEAMENTO RIGOROSO DAS 25 COLUNAS (A-Y)
    ...
    with tab_at:
        ...
        with c1:
            ...
            p_mg_des = st.number_input("Mg% desejado CTC", 18.0); p_preco_c = st.number_input("Preço Calcário (t)", 190.0)
            p_adicional = st.number_input("Adicional de Calcário (t/ha)", 0.0)
        with c2:
