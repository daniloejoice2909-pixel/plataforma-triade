...
# Paleta de 6 Camadas: Azul (Alto/Suficiente) ao Vermelho (Crítico/Baixo)
...
if st.session_state.pagina == "Entrada":
    ...
    st.header("📂 Cadastro e Importação A a Y")
    c1, c2 = st.columns(2)
    with c1:
        st.session_state.info['produtor'] = st.text_input("Nome do Produtor", "Danilo")
        st.session_state.info['fazenda'] = st.text_input("Nome da Fazenda", "Berneck")
    ...
    # --- ABA 1: ATRIBUTOS ---
    with tabs[0]:
        st.subheader("Configuração Global de Recomendações")
        ...
        with c1:
            ...
            p_mg_des = st.number_input("Mg% desejado CTC", 18.0); p_preco_c = st.number_input("Preço Calcário", 190.0)
        ...
        with c2:
            ...
            p_exp_p_f = st.number_input("Exp. P (kg/sc)", 0.8)
        ...
        with c3:
            ...
            p_perc_k = st.number_input("% K2O no Adubo", 60.0); g_f = st.number_input("Fator Gesso (Arg * F)", 15)
    ...
    with tabs[1]:
        sel_f = st.selectbox("Escolha Fertilidade:", ['P', 'K', 'PH', 'ARGILA', 'PREM', 'CTC', 'CA', 'MG'])
        if st.button("Gerar Mapa"): render_mapa_triade(sel_f, f"Fertilidade: {sel_f}", "Variabilidade nutricional.")
    ...
    with tabs[2]:
        ...
        # Fósforo com abatimento de gordura
        ...
        # Potássio com soma incondicional de exportação
        ...
        sel_vr = st.selectbox("Selecione Insumo:", ['REC_CALC', 'REC_P_ADUBO', 'REC_K_ADUBO', 'REC_GESSO'])
        if st.button("Gerar Recomendação"):
            just = f"Metodologia estratégica Tríade Agro aplicada para {sel_vr}."
            render_mapa_triade(sel_vr, sel_vr, just)
    ...
    with tabs[3]:
        ...
        with c1:
            if st.button("📥 GERAR RELATÓRIO PDF"):
                ...
                f_n = st.session_state.info.get('fazenda', 'Geral')
                p_n = st.session_state.info.get('produtor', 'Berneck')
                ...
                st.download_button("Baixar PDF", bytes(pdf.output()), f"Relatorio_Triade_{f_n}.pdf", "application/pdf")
        with c2:
