...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 10, 'RELATÓRIO ESTRATÉGICO DE PRESCRIÇÃO - TRÍADE AGRO', 0, 1, 'C')
        self.ln(10)
...
if st.session_state.pagina == "Entrada":
    ...
    # --- GLOBALIZAÇÃO DOS ATRIBUTOS (Resolve o NameError) ---
    st.sidebar.header("⚙️ Configurações Ativas")
    with st.sidebar.expander("Calcário (CaO 36 / MgO 9)", expanded=False):
        p_cao = st.number_input("CaO %", 36.0); p_mgo = st.number_input("MgO %", 9.0)
        p_prnt = st.number_input("PRNT %", 80.0); p_ca_des = st.number_input("Ca% desejado", 60.0)
        p_mg_des = st.number_input("Mg% desejado", 18.0); p_adicional = st.number_input("Adicional (t/ha)", 0.0)
    with st.sidebar.expander("Fósforo e Textura", expanded=False):
        nc_list = [st.number_input(f"NC Classe {i+1}", v) for i, v in enumerate([8.0, 10.0, 12.0, 15.0, 20.0, 25.0])]
        f_marg = st.number_input("Fator M.Arg", 10.0); f_arg = st.number_input("Fator Arg", 8.0)
    ...
    with st.sidebar.expander("Potássio e Gesso", expanded=False):
        ...
        g_fator = st.number_input("Fator Gesso", 15); g_max = st.number_input("Dose Máx Gesso", 900.0)
    ...
    def render_mapas_grid(colunas, desc_base, is_recom=False):
        n_cols = 3
        rows = [colunas[i:i + n_cols] for i in range(0, len(colunas), n_cols)]
        for row in rows:
            st_cols = st.columns(3)
            for i, col_id in enumerate(row):
                with st_cols[i]:
                    ...
                    ax.plot(x_p, y_p, color='black', linewidth=3) # CONTORNO PRETO
                    
                    buf = io.BytesIO(); plt.savefig(buf, format='png', transparent=True, bbox_inches='tight', pad_inches=0); plt.close(fig)
                    st.image(buf, use_container_width=True)
                    st.markdown(f"<p style='text-align:center; font-size:11px;'><b>{col_id}</b><br>Mín: {v_min:.2f} | Méd: {v_med:.2f} | Máx: {v_max:.2f}</p>", unsafe_allow_html=True)
                    st.session_state.mapas_finalizados[col_id] = {"img": buf, "stats": f"Máx: {v_max:.2f} | Méd: {v_med:.2f} | Mín: {v_min:.2f}", "titulo": col_id, "desc": desc_base}

    with tabs[0]:
        render_mapas_grid(['P', 'K', 'PH', 'ARGILA', 'PREM', 'CTC'], "Diagnóstico de Fertilidade Tríade Agro.")
    ...
    with tabs[1]:
        ...
        df['REC_K_ADUBO'] = (((((p_k_des - df['K_PERC']).clip(lower=0) * df['CTC'] / 100) * 391 * 2 * 1.2) + (p_prod * p_exp_k * 1.2)) * 100 / 60).round(2)
        # 4. Gesso
        df['REC_GESSO'] = (df['ARGILA'] * g_fator / 10).clip(lower=400, upper=g_max)

        render_mapas_grid(['REC_CALCARIO', 'REC_P_ADUBO', 'REC_K_ADUBO', 'REC_GESSO'], "Recomendação Estratégica VRA.")
    ...
    with tabs[2]:
        ...
        with c2:
            if st.button("🚜 GERAR ZIP MÁQUINAS (Multi-Marcas)"):
                zip_b = io.BytesIO()
                with zipfile.ZipFile(zip_b, "w") as z:
                    z.writestr("John_Deere/prescricao.shp", "Dados espaciais"); z.writestr("Case_IH/prescricao.xml", "
