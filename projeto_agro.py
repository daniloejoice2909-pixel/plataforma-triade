...
# --- CONFIGURAÇÃO DA PÁGINA (OPEN SANS / A4) ---
...
# --- MOTOR DE CÁLCULO TRÍADE V43 (REGRAS INTEGRAIS) ---
def motor_calculo_v43(df, params):
    # 1. Normalização Blindada de Colunas
    ...
    # Garantir existência de colunas Latitude/Longitude
    ...
    # --- CÁLCULOS ---
    # 1. Calagem Atômica (Maior entre Ca 560 / Mg 400 + Reserva)
    ...
    # 2. Fósforo (Balanço com 6 faixas de P-rem e Crédito de Solo)
    def calc_p(row):
        pr = row['prem']
    ...
    # 3. Potássio (Correção + Exportação) e Gesso (Argila % * 10)
...
def configurar_interface():
    ...
    with st.sidebar.expander("🌍 Atributos Tríade v43", expanded=True):
        ...
        st.write("**Fósforo (6 Faixas P-rem)**")
    ...
    with tabs[0]:
        ...
        with c1:
            ...
            if st.button("🚀 Processar"):
                if up_csv: 
                    st.session_state['db'][p] = {
                        "df": pd.read_csv(up_csv, sep=None, engine='python'), 
                        "contorno": json.load(up_geo) if up_geo else None
                    }
                    st.success("Dados vinculados!")

    # ACESSO BLINDADO AO BANCO DE DADOS
    if st.session_state['db'].get(p) is not None:
        df_raw = st.session_state['db'][p].get("df")
                ...
                for i in range(0, len(attrs), 2):
                    ...
                    for j in range(2):
                        if i+j < len(attrs):
                            ...
                            cols[j].plotly_chart(fig, use_container_width=True, key=f"f_{attrs[i+j]}")
                            cols[j].info(stats)

            with tabs[2]: # VRT
                recs = [("REC_CALCARIO", "Calcário"), ("REC_P_ADUBO", "Fosfatado"), ("REC_K_ADUBO", "Potássico"), ("REC_GESSO", "Gesso")]
                args_tec = {"Calcário": "Máxima eficiência via equilíbrio estequiométrico individual.", "Fosfatado": "NC via P-rem com crédito de solo.", "Potássio": "Saturação ideal + exportação real.", "Gesso": "Melhoria radicular baseada em Argila %."}
                for i in range(0, len(recs), 2):
                    cols = st.columns(2)
                    for j in range(2):
                        if i+j < len(recs):
                            fig, stats = plot_geostats(df_res, recs[i+j][0], f"VRT {recs[i+j][1]}", contorno)
                            cols[j].plotly_chart(fig, use_container_width=True, key=f"v_{recs[i+j][1]}")
                            cols[j].success(stats)
                            cols[j].markdown(f"<div class='arg-tecnico'><b>Argumento:</b> {args_tec.get(recs[i+j][1])}</div>", unsafe_allow_html=True)

# --- EXECUÇÃO ---
params = configurar_interface()
p, f, t = params["path"]
if p and p != "+ Novo Produtor": pag_produtores(params)
else: st.info("Selecione ou crie um produtor na lateral.")
