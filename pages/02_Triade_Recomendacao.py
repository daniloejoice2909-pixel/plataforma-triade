...
def gerar_mapa_app1(df, atributo, titulo, geojson_data):
    ...
    folium.raster_layers.ImageOverlay(
        image=f"data:image/png;base64,{img_b64}", bounds=[[Y.min(), X.min()], [Y.max(), X.max()]], opacity=0.85
    ).add_to(m)
...
with st.sidebar:
    ...
    with st.expander("⚪ 2. Calagem", expanded=False):
        ...
        teor_cao = st.number_input("CaO (%):", value=38.0)
        teor_mgo = st.number_input("MgO (%):", value=12.0)
        prnt = st.number_input("PRNT (%):", value=85.0)
    with st.expander("🔴 3. Fósforo (Tabela Fixa)", expanded=True):
        p_export = st.number_input("Exp. P (kg/sc):", value=0.8)
        p_teor = st.number_input("Teor P2O5 (%):", value=52.0)
        st.write("Níveis Críticos (mg/dm³)")
        c1, c2 = st.columns(2)
        with c1:
            nc_1 = st.number_input("0-4", value=5.5)
            nc_2 = st.number_input("4-10", value=8.0)
            nc_3 = st.number_input("10-19", value=12.0)
        with c2:
            nc_4 = st.number_input("19-30", value=15.0)
    ...
    with st.expander("🟣 4. Potássio", expanded=False):
        k_alvo_ctc = st.number_input("K Alvo (%):", value=3.0)
        k_export = st.number_input("Exp. K (kg/sc):", value=1.2)
...
if st.button("🚀 Gerar Mapas", type="primary"):
    with st.spinner("Processando..."):
        if df_input is not None:
            ...
            st.rerun()
        else: st.error("Carregue os arquivos.")
